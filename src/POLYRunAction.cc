...
//
// POLYRunAction.cc
// \file   MRCP_GEANT4/External/src/POLYRunAction.cc
// \author Haegin Han
//
...
POLYRunAction::POLYRunAction(POLYModelImport* _POLYData, G4String _output, G4Timer* _init)
:POLYData(_POLYData), fRun(0), numOfEvent(0), runID(0), outputFile(_output), initTimer(_init), runTimer(0)
{
	if(isMaster){
		runTimer = new G4Timer;
		std::ofstream ofs(outputFile);
		auto massMap = POLYData->GetMassMap();
		ofs<<"run#\tinitT\trunT\t";
...
POLYRunAction::~POLYRunAction()
{}

G4Run* POLYRunAction::GenerateRun()
{
	// generate run
	fRun = new POLYRun();
	return fRun;
...
void POLYRunAction::BeginOfRunAction(const G4Run* aRun)
{
...
void POLYRunAction::EndOfRunAction(const G4Run* aRun)
{
...
void POLYRunAction::PrintResult(std::ostream &out)
{
	// Print run result
	//t
	using namespace std;
...
void POLYRunAction::PrintLine(std::ostream &out)
{
	// Print run result
	//
	using namespace std;
	...
	out << runID << "\t"<< initTimer->GetRealElapsed() << "\t"<< runTimer->GetRealElapsed()<<"\t";
	auto massMap = POLYData->GetMassMap();
	for(auto itr : massMap){
		G4double meanDose    = edepMap[itr.first].first  / itr.second / numOfEvent;
		G4double squareDoese = edepMap[itr.first].second / (itr.second*itr.second);
		...
		out << meanDose*1e12/(joule/kg) <<"\t" << relativeE << "\t";
	}
	out<<G4endl;
}
